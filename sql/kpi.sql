...
SELECT
    ROUND( ( COUNT(CASE WHEN asistencia = TRUE THEN 1 END)::DECIMAL / COUNT(*)) * 100, 2) AS tasa_asistencia
FROM sesiones;

-- 4. Efectividad promedio
SELECT ROUND( AVG( ( nivel_movilidad + resistencia + (10 - nivel_dolor)) / 3.0), 2
    ) AS efectividad_promedio
...
FROM vehiculos v
JOIN cat_estados_vehiculo cev
    ON v.id_estado = cev.id_estado
WHERE cev.nombre = 'EN_USO';

-- 7. Dispositivos NFC activos
SELECT COUNT(*) AS dispositivos_nfc_activos
FROM dispositivos_nfc d
JOIN cat_estados_dispositivo ced
    ON d.id_estado = ced.id_estado
...
FROM terapeutas t
JOIN terapeuta_sesion ts
    ON t.id_terapeuta = ts.id_terapeuta
GROUP BY
    t.id_terapeuta,
    t.nombre,
    t.apellido_paterno
...
SELECT
    ROUND(AVG(nivel_dolor), 2) AS promedio_dolor
FROM evaluaciones_progreso;

-- 10. Promedio de movilidad
SELECT
    ROUND(AVG(nivel_movilidad), 2) AS promedio_movilidad
FROM evaluaciones_progreso;
...
JOIN evaluaciones_progreso ep
    ON pe.id_evaluacion = ep.id_evaluacion
WHERE ep.tipo_evaluacion = 'Final';


-- 12. Tiempo promedio por sesion
SELECT 
  ROUND( AVG( EXTRACT(EPOCH FROM (hora_fin_real - hora_inicio_real)) / 60), 2

-- =============================================================================
-- SCHEMA CONSOLIDADO
-- Base: respaldo_sergio | Reconciliado con: respaldo_javier
-- Secciones: DDL → Funciones → Triggers → Vistas → SPs → DML
-- =============================================================================
...
-- ----------------------------------------------------------------------------
-- NOTA: character(20) adoptado de sergio; javier usaba character(12)
...
-- ----------------------------------------------------------------------------
-- NOTA: asistencia sin DEFAULT — sergio eliminó DEFAULT false
...
-- ----------------------------------------------------------------------------
-- NOTA: genera login 'fam_N'. Registros históricos usan 'fam_pacN';
--       ON CONFLICT DO NOTHING absorbe colisiones pero la inconsistencia
--       de nomenclatura persiste para registros anteriores al cambio.
...
-- ----------------------------------------------------------------------------
-- NOTA: columna score_progreso agregada en sergio
...
-- ----------------------------------------------------------------------------
-- NOTA: sergio elimina estado_sesion, hora_inicio_real, hora_fin_real
...
-- ----------------------------------------------------------------------------
-- NOTA: presente solo en javier; compatible con vw_sesiones_detalle actual
...
SELECT
    ...
    round(
        (count(CASE WHEN s.asistencia = true THEN 1 ELSE NULL::integer END)::numeric
         / NULLIF(count(ps.id_sesion), 0)::numeric) * 100::numeric, 1
...
LANGUAGE plpgsql AS $$
BEGIN
    SELECT TRUE, u.id_usuario, u.rol,
        CASE u.rol
            WHEN 'terapeuta' THEN (SELECT t.nombre FROM terapeutas t WHERE t.id_terapeuta = u.ref_id)
            WHEN 'familiar'  THEN (SELECT p.nombre FROM pacientes  p WHERE p.id_paciente  = u.ref_id)
            ELSE 'Administrador'
        END,
        u.ref_id
...
END;
$$;
ALTER PROCEDURE public.sp_login(VARCHAR,VARCHAR,BOOLEAN,INTEGER,VARCHAR,VARCHAR,INTEGER) OWNER TO postgres;
...
END;
$$;
ALTER PROCEDURE public.sp_kpis_terapeuta(INTEGER,DATE,BIGINT,BIGINT,BIGINT,BIGINT,BIGINT) OWNER TO postgres;

-- ----------------------------------------------------------------------------

CREATE OR REPLACE PROCEDURE public.sp_insertar_paciente(
    IN p_nombre       VARCHAR, IN p_ap_paterno VARCHAR, IN p_ap_materno VARCHAR,
    IN p_fecha_nac    DATE,    IN p_id_sexo    INTEGER,
    IN p_telefono     VARCHAR, IN p_correo     VARCHAR, IN p_direccion  VARCHAR,
    IN p_diagnostico  VARCHAR, IN p_antecedentes TEXT
)
LANGUAGE plpgsql AS $$
BEGIN
    INSERT INTO pacientes (
        nombre, apellido_paterno, apellido_materno, fecha_nacimiento, id_sexo,
        telefono, correo, direccion, diagnostico_principal, antecedentes_medicos
    ) VALUES (
        p_nombre, p_ap_paterno, p_ap_materno, p_fecha_nac, p_id_sexo,
        p_telefono, p_correo, p_direccion, p_diagnostico, p_antecedentes
    );
END;
$$;
ALTER PROCEDURE public.sp_insertar_paciente(VARCHAR,VARCHAR,VARCHAR,DATE,INTEGER,VARCHAR,VARCHAR,VARCHAR,VARCHAR,TEXT) OWNER TO postgres;
...
END;
$$;
ALTER PROCEDURE public.sp_actualizar_paciente(INTEGER,VARCHAR,VARCHAR,VARCHAR,DATE,INTEGER,VARCHAR,VARCHAR,VARCHAR,VARCHAR,TEXT) OWNER TO postgres;
...
CREATE OR REPLACE PROCEDURE public.sp_insertar_terapeuta(
    ...
    IN p_telefono        VARCHAR, IN p_correo     VARCHAR, IN p_observaciones TEXT
)
LANGUAGE plpgsql AS $$
BEGIN
...
END;
$$;
ALTER PROCEDURE public.sp_insertar_terapeuta(VARCHAR,VARCHAR,VARCHAR,INTEGER,VARCHAR,VARCHAR,TEXT) OWNER TO postgres;
...
END;
$$;
ALTER PROCEDURE public.sp_actualizar_terapeuta(INTEGER,VARCHAR,VARCHAR,VARCHAR,INTEGER,VARCHAR,VARCHAR,TEXT) OWNER TO postgres;
...
CREATE OR REPLACE PROCEDURE public.sp_registrar_sesion(
    IN p_id_paciente   INTEGER, IN p_id_terapeuta INTEGER,
    IN p_fecha         DATE,
    IN p_hora_inicio   TIME,    IN p_hora_fin     TIME,
    IN p_id_tipo       INTEGER, IN p_asistencia   BOOLEAN,
    IN p_id_metodo     INTEGER, IN p_observaciones TEXT
)
LANGUAGE plpgsql AS $$
DECLARE
    v_id_sesion INTEGER;
BEGIN
    ...
    INSERT INTO sesiones (
        fecha, hora_inicio, hora_fin, id_tipo_sesion,
        observaciones_clinicas, asistencia, id_metodo, estado_sesion
    ) VALUES (
        p_fecha, p_hora_inicio, p_hora_fin, p_id_tipo,
        p_observaciones, p_asistencia, p_id_metodo, 'PROGRAMADA'
    )
...
END;
$$;
ALTER PROCEDURE public.sp_registrar_sesion(INTEGER,INTEGER,DATE,TIME,TIME,INTEGER,BOOLEAN,INTEGER,TEXT) OWNER TO postgres;

-- ----------------------------------------------------------------------------

CREATE OR REPLACE PROCEDURE public.sp_actualizar_sesion(
    IN p_id_sesion    INTEGER, IN p_fecha       DATE,
    IN p_hora_inicio  TIME,    IN p_hora_fin    TIME,
    IN p_id_tipo      INTEGER, IN p_asistencia  BOOLEAN,
    IN p_id_metodo    INTEGER, IN p_observaciones TEXT
)
...
END;
$$;
ALTER PROCEDURE public.sp_actualizar_sesion(INTEGER,DATE,TIME,TIME,INTEGER,BOOLEAN,INTEGER,TEXT) OWNER TO postgres;

-- ----------------------------------------------------------------------------
-- NOTA: borra asistencias_nfc primero para evitar FK RESTRICT sobre id_sesion
...
-- Orden: usuarios → terapeutas → pacientes → dependientes
-- Los triggers trg_crear_usuario_* están activos pero usuarios ya existe;
-- los logins que generen (ter_1, ter_2, fam_1..fam_9) no colisionan con los
-- logins manuales (laura.mendoza, andres.fuentes, fam_pac1, fam_pac2, fam_9)
-- porque ON CONFLICT DO NOTHING los descarta.
-- =============================================================================

-- 1. usuarios — debe existir antes de que disparen los triggers
...
-- 2. terapeutas — trigger genera ter_1, ter_2 → ON CONFLICT DO NOTHING los descarta
...
-- 3. pacientes — trigger genera fam_1..fam_9 → ON CONFLICT DO NOTHING descarta fam_9
...
-- 7. datos clínicos del paciente
INSERT INTO public.paciente_alergia (id_paciente, alergia) VALUES
(1, 'Penicilina'), (3, 'Ibuprofeno'), (3, 'Látex'),
(5, 'Aspirina'),   (7, 'Sulfamidas');
...
(1, 'Naproxeno 500mg'),       (3, 'Metformina 850mg'),
(3, 'Losartán 50mg'),          (5, 'Ácido hialurónico intraarticular'),
(7, 'Prednisolona 5mg');

-- 8. dispositivos e IoT
...
VALUES
(1,  4, 1, '2024-11-04', '08:52:00'), (2,  1, 1, '2024-11-04', '09:00:00'),
(3,  4, 2, '2024-11-04', '09:54:00'), (4,  2, 2, '2024-11-04', '10:00:00'),
(5,  4, 3, '2024-11-05', '08:25:00'), (6,  1, 3, '2024-11-05', '08:30:00'),
(7,  3, 5, '2024-11-06', '09:10:00'), (8,  5, 7, '2024-11-06', '09:02:00'),
(9,  4, 8, '2024-11-07', '07:55:00'), (10, 2, 8, '2024-11-07', '08:00:00');
